...
const SECRET = "95715731c840405fb598b2640a161012";
...
const getGameMedia = async (id, type) => {
    let promise = await fetch(`https://api.rawg.io/api/games/${id}/${type}?key=${SECRET}`)
...
const appendScreenshots = (arr) => {
    ...
    console.log(arr.screenshots_array)
    for (let i = 0; i <= 3; i++) {
...
const appendVideo = (arr) => {
    ...
    let videoToAppend = "";
    console.log(arr.video_array, 'aca');
    console.log(arr.video_array.length == false);
...
const createModalGame = (gameObj) => {
    console.log(gameObj.screenshots_array);
        ...
        <div class="modal-background-filter">
            <div class="a">
                ...
                <div class="modal-game-platforms">
                    ${platformAppend(gameObj.platforms)}
                </div>
            ...
            <div class="b">
                ...
                <div class="b2">
                    ...
                    <div class="modal-game-information">
                        ...
                        <a href="#" class="modal-game-link underline">${gameObj.website}</a>
                    </div>
                </div>
                <div class="b3">
                    <div class="modal-game-information">
...
const changeModalBackgroundImage = (gameObj) => {
    console.log(gameObj.background_image);
...
const getJSONData = async (page) => {
    ...
    let promise = await fetch(`${RAWG_API_URL}?page_size=${cardsPerPage}&page=${page}&key=${SECRET}`);
    let result = await promise.json();

    for (let element of result.results) {
        let promise = await fetch(`${RAWG_API_URL}/${element.id}?key=${SECRET}`);
        let result = await promise.json();
    ...
    for (let element of arr) {
        ...
        <div class="card-information">
            ...
            <div class="card-description-container">
                <p class="card-description-text">${element.description_raw}</p>
            </div>
        </div>
    </div>
                `
...
const searchGames = async () => {
    let promise = await fetch(`${RAWG_API_URL}?key=${SECRET}&search=${searchBox.value}`);
    let result = await promise.json();
